...
interface SessionQuestionRepository : JpaRepository<SessionQuestion, Long> {
    ...
    /**
     * 자동 넘김 기한이 온 문항 — 마감된 지 결과 표시 시간이 지났고, 방이 아직 그 문항에 머물러 있고,
     * 열리지 않은 다음 문항이 있는 것만. 마지막 문항·호스트가 이미 넘긴 방은 걸리지 않아
    ...
    @org.springframework.data.jpa.repository.Query(
        ...
        where sq.autoAdvance = true
          ...
          and exists (
              select 1 from SessionQuestion next
              where next.roomId = sq.roomId and next.orderNo = sq.orderNo + 1 and next.startedAt is null
          )
        """,
    )
