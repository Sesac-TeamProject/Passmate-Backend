...
> Redis는 후순위. 1차 MVP는 MySQL만으로 동작한다 ([CLAUDE.md](CLAUDE.md) 참고)
...
호스트 3306 이 이미 쓰이면(macOS 네이티브 MySQL 등) `.env` 에 `MYSQL_HOST_PORT=3307` · `DB_PORT=3307` 을 넣는다 — compose 와 앱이 함께 따라간다.
...
| 문서 | 위치 |
|---|---|
| 작업 규칙 (아키텍처 · 컨벤션 · 금지사항) | [CLAUDE.md](CLAUDE.md) |
...
```
Passmate-Backend/
├── docker-compose.local.yml   MySQL 8 (Redis 후순위)
├── Dockerfile                 배포 이미지 (JDK 17)
