...
- **커밋은 되돌릴 수 있는 단위로 쪼갠다.** 한 커밋 = 한 관심사, 그리고 **각 커밋은 그 자체로 빌드가 되어야** 롤백 지점이 된다
  - **레이어 순서로 나눈다**: `db:`(Flyway + ERD) → 도메인(엔티티 + 단위 테스트) → 서비스(+ 테스트) → 컨트롤러·DTO(+ 통합 테스트) → 다른 기능에 반영(예: 세션 시작이 방 설정을 읽게 하기). 한 파일을 두 커밋에 걸쳐 고쳐야 하면 **중간 상태를 만들어서라도** 나눈다
  - **기준선: 파일 5개 또는 150줄을 넘으면 쪼갤 자리부터 찾는다.** 10개·500줄은 상한이지 목표가 아니다 — 그 아래여도 관심사가 둘이면 쪼갠다
  - 작업을 시작할 때 **커밋 목록을 먼저 적고** 그 순서대로 구현한다. 커밋 직전에 `git diff --stat` 으로 파일·줄 수와 관심사 수를 다시 센다
  - 어긴 사례(반복 금지): 2026-09-03 PR #21~28(PR당 커밋 1개, 최대 24파일·1062줄) · 2026-09-07 B-18~23(커밋 5개가 각각 관심사 2~3개 — 스키마+도메인, 서비스+컨트롤러+타 기능 반영을 한 커밋에)
- 테스트는 대상 코드와 **같은 커밋**에 넣는다. 테스트만 따로 커밋하면 롤백 지점이 '테스트 없는 상태'가 된다. 통합 테스트 하나가 여러 커밋을 검증하면 **단정문을 커밋별로 나눠 넣는다**
...
- **머지했으면 그 브랜치는 지운다** — 원격·로컬 둘 다(`gh pr merge --delete-branch` + `git fetch -p`). 남은 브랜치 목록은 "지금 진행 중인 작업"만 보여야 한다
  - 단, **다른 워크트리가 체크아웃한 브랜치는 건드리지 않는다**(`git branch -vv` 에 `+` 로 표시된다). 다른 세션이 그 위에서 작업 중일 수 있다
  - 지우기 전에 `git branch -r --merged develop` 로 **정말 머지됐는지 확인한다.** 확인과 삭제 사이에 커밋이 들어올 수 있으니 삭제 직전에 다시 본다
- **PR 크기는 "화면 한 묶음" 정도로** — API 1개씩 쪼개지도, 우선순위 전체를 한 PR에 몰지도 않는다. 관련된 엔드포인트 2~4개(예: 별점 제출+목록, 세트 복제+삭제)가 한 PR
